...
/* SWIPER STUFF */
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize Swiper after media blocks have been initialized
        initializeSwiper();

        // Handle Tab Visibility Changes
        handleTabVisibility();

        // Set up periodic refresh of iframes
        const refreshInterval = 5 * 60 * 1000; // Refresh every 5 minutes
        setInterval(refreshVimeoIframes, refreshInterval);
        ...
        // Fade blur animation on page load
        // Delay the start of the animation by 0.5 seconds
        setTimeout(function() {
            // Get elements to animate
            const elementsToAnimate = document.querySelectorAll('.mobile-body, .swiper-block');
    ...
    function fetchVimeoThumbnail(vimeoUrl, mediaBlock) {
        ...
        if (cachedThumbnail) {
            injectVimeoPlaceholder(mediaBlock, cachedThumbnail);
            injectVimeoIframe(mediaBlock, vimeoUrl); // Proceed to inject iframe
            return;
        }
    ...
    /* Function to Inject Vimeo Placeholder with Thumbnail */
    function injectVimeoPlaceholder(mediaBlock, thumbnailUrl) {
        const placeholder = document.createElement('div');
        placeholder.classList.add('placeholder');
    ...
    /* Function to Refresh Vimeo Iframes */
    function refreshVimeoIframes() {
    ...
    function handleTabVisibility() {
        document.addEventListener('visibilitychange', function() {
            if (document.visibilityState === 'visible') {
                // Tab has become visible, refresh iframes
                refreshVimeoIframes();
            }
        });
    }
